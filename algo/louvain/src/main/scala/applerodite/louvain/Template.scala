...
// 定义算法输入
case class Param(`maxIter`: Integer, `internalIter`: Integer, `tol`: Double, graphID: String, target: String)

// 辅助约束算法输出

case class ResultRow(`id`: Long, `class`: String) {
  def toRow(`id`: String = this.`id`.toString, `class`: String = this.`class`.toString): Row = {
    Row.apply(`id`, `class`)
  }
}
...
abstract class Template {
  ...
  def input(args: Array[String]): Param = {
	...
	val `maxIter`: Integer =json.getInteger("maxIter")
val `internalIter`: Integer =json.getInteger("internalIter")
val `tol`: Double =json.getDouble("tol")
Param(maxIter = maxIter, internalIter = internalIter, tol = tol, graphID = graphID, target = target)
  }
