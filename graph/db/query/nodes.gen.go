...
func newNode(db *gorm.DB, opts ...gen.DOOption) node {
	...
	_node.GroupID = field.NewInt64(tableName, "groupID")
	_node.Name = field.NewString(tableName, "name")
	_node.Desc = field.NewString(tableName, "desc")
	_node.Display = field.NewString(tableName, "display")
	_node.NodeAttrs = nodeHasManyNodeAttrs{
		db: db.Session(&gorm.Session{}),

		RelationField: field.NewRelation("NodeAttrs", "model.NodesAttr"),
	}
...
type node struct {
	...
	NodeAttrs nodeHasManyNodeAttrs
...
func (n *node) updateTableName(table string) *node {
	...
	n.GroupID = field.NewInt64(table, "groupID")
	n.Name = field.NewString(table, "name")
	n.Desc = field.NewString(table, "desc")
	n.Display = field.NewString(table, "display")
...
func (n *node) fillFieldMap() {
	...
	n.fieldMap["id"] = n.ID
	n.fieldMap["groupID"] = n.GroupID
	n.fieldMap["name"] = n.Name
	n.fieldMap["desc"] = n.Desc
	n.fieldMap["display"] = n.Display
...
type nodeHasManyNodeAttrs struct {
	db *gorm.DB

	field.RelationField
}

func (a nodeHasManyNodeAttrs) Where(conds ...field.Expr) *nodeHasManyNodeAttrs {
	if len(conds) == 0 {
		return &a
	}
...
func (a nodeHasManyNodeAttrs) WithContext(ctx context.Context) *nodeHasManyNodeAttrs {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a nodeHasManyNodeAttrs) Session(session *gorm.Session) *nodeHasManyNodeAttrs {
	a.db = a.db.Session(session)
	return &a
}

func (a nodeHasManyNodeAttrs) Model(m *model.Node) *nodeHasManyNodeAttrsTx {
	return &nodeHasManyNodeAttrsTx{a.db.Model(m).Association(a.Name())}
}

type nodeHasManyNodeAttrsTx struct{ tx *gorm.Association }

func (a nodeHasManyNodeAttrsTx) Find() (result []*model.NodesAttr, err error) {
	return result, a.tx.Find(&result)
}

func (a nodeHasManyNodeAttrsTx) Append(values ...*model.NodesAttr) (err error) {
...
func (a nodeHasManyNodeAttrsTx) Replace(values ...*model.NodesAttr) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a nodeHasManyNodeAttrsTx) Delete(values ...*model.NodesAttr) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a nodeHasManyNodeAttrsTx) Clear() error {
	return a.tx.Clear()
}

func (a nodeHasManyNodeAttrsTx) Count() int64 {
	return a.tx.Count()
}
