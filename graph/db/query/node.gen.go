...
func newNode(db *gorm.DB, opts ...gen.DOOption) node {
	...
	_node.Primary = field.NewString(tableName, "primary")
	_node.Attrs = nodeHasManyAttrs{
		db: db.Session(&gorm.Session{}),

		RelationField: field.NewRelation("Attrs", "model.NodeAttr"),
	}
...
func (n *node) updateTableName(table string) *node {
	...
	n.Primary = field.NewString(table, "primary")
...
type nodeHasManyAttrs struct {
	db *gorm.DB

	field.RelationField
}

func (a nodeHasManyAttrs) Where(conds ...field.Expr) *nodeHasManyAttrs {
	if len(conds) == 0 {
		return &a
	}
...
func (a nodeHasManyAttrs) WithContext(ctx context.Context) *nodeHasManyAttrs {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a nodeHasManyAttrs) Session(session *gorm.Session) *nodeHasManyAttrs {
	a.db = a.db.Session(session)
	return &a
}

func (a nodeHasManyAttrs) Model(m *model.Node) *nodeHasManyAttrsTx {
	return &nodeHasManyAttrsTx{a.db.Model(m).Association(a.Name())}
}

type nodeHasManyAttrsTx struct{ tx *gorm.Association }

func (a nodeHasManyAttrsTx) Find() (result []*model.NodeAttr, err error) {
	return result, a.tx.Find(&result)
}

func (a nodeHasManyAttrsTx) Append(values ...*model.NodeAttr) (err error) {
...
func (a nodeHasManyAttrsTx) Replace(values ...*model.NodeAttr) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a nodeHasManyAttrsTx) Delete(values ...*model.NodeAttr) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a nodeHasManyAttrsTx) Clear() error {
	return a.tx.Clear()
}

func (a nodeHasManyAttrsTx) Count() int64 {
	return a.tx.Count()
}
