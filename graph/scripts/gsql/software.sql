...
INSERT INTO graph.algo(id, name, `desc`, detail, groupId, tag, jarPath, mainClass) VALUES (1, 'quantity', '广度排序算法，基于假设：软件的入度越大越重要。使用Voterank算法衡量节点的入度的相对大小。', '广度排序算法，基于假设：软件的入度越大越重要。使用Voterank算法衡量节点的入度的相对大小。', 2, '软件影响力', 's3a://lib/quantity-latest.jar', 'applerodite.Main');


INSERT INTO graph.algo(id, name, `desc`, detail, groupId, tag, jarPath, mainClass) VALUES (2, 'depth', '深度排序算法，基于假设：在更多依赖路径中处于尾部的软件更重要。', '深度排序算法，基于假设：在更多依赖路径中处于尾部的软件更重要。', 2, '软件影响力', 's3a://lib/depth-latest.jar', 'applerodite.Main');


INSERT INTO graph.algo(id, name, `desc`, detail, groupId, tag, jarPath, mainClass) VALUES (3, 'mediation', '中介度排序算法，基于假设：在更多依赖路径中处于中部的软件具有重要的集成作用。使用betweenness算法衡量软件的中介作用。', '中介度排序算法，基于假设：在更多依赖路径中处于中部的软件具有重要的集成作用。使用betweenness算法衡量软件的中介作用。', 2, '软件影响力', 's3a://lib/mediation-latest.jar', 'applerodite.Main');


INSERT INTO graph.algo(id, name, `desc`, detail, groupId, tag, jarPath, mainClass) VALUES (4, 'stability', '稳定性排序算法，基于假设：对软件开发生态稳定贡献更大的软件具有更高的影响力。使用基于最小渗流的collective influence算法衡量软件对生态稳定的贡献。', '稳定性排序算法，基于假设：对软件开发生态稳定贡献更大的软件具有更高的影响力。使用基于最小渗流的collective influence算法衡量软件对生态稳定的贡献。', 2, '软件影响力', 's3a://lib/stability-latest.jar', 'applerodite.Main');


INSERT INTO graph.algo(id, name, `desc`, detail, groupId, tag, jarPath, mainClass) VALUES (5, 'hhi', '“hhi”指数是赫芬达尔—赫希曼指数 (Herfindahl-Hirschman Index)的简称，它是一种测量产业集中度的综合指数，被用于衡量同个主题下的软件的垄断程度。', '“hhi”指数是赫芬达尔—赫希曼指数 (Herfindahl-Hirschman Index)的简称，它是一种测量产业集中度的综合指数。它反映了一个行业中各市场竞争主体所占行业总收入或总资产百分比的平方和，用来计量市场份额的变化，即市场中厂商规模的离散度。它是经济学界和政府管制部门使用较多的指标。在此处，“hhi”指数被应用衡量同个主题下的软件的垄断程度。
...
', 2, '网络拓扑特性', 's3a://lib/hhi-latest.jar', 'applerodite.Main');
