...
import AlgoDoc from "@/pages/AlgoDoc";
...
export function render(oldRender: () => void) {
    ...
    const getAlgo = algoGetAll().then(res => {
        initAlgos = res.algos
        newAlgoRoutes = Object.entries<Graph.Algo[]>(initAlgos.reduce((group: any, algo) => {
            group[algo.tag] = [...(group[algo.tag] ?? []), algo]
            return group
        }, {})).sort((a, b) => a[0].localeCompare(b[0])).map(
            (e) => {
                // 取出当前TAG下的第一个算法的TAGID作为路由，该TAG下若没有算法，这里也不会显示
                const tagId = e[1][0].tagId
                return {
                    path: `/algo/${tagId}`,
                    name: e[0],
                    children: [
