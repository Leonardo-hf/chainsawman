...
func newEdge(db *gorm.DB, opts ...gen.DOOption) edge {
	...
	_edge.Primary = field.NewString(tableName, "primary")
	_edge.Attrs = edgeHasManyAttrs{
		db: db.Session(&gorm.Session{}),

		RelationField: field.NewRelation("Attrs", "model.EdgeAttr"),
	}
...
func (e *edge) updateTableName(table string) *edge {
	...
	e.Primary = field.NewString(table, "primary")
...
type edgeHasManyAttrs struct {
	db *gorm.DB

	field.RelationField
}

func (a edgeHasManyAttrs) Where(conds ...field.Expr) *edgeHasManyAttrs {
	if len(conds) == 0 {
		return &a
	}
...
func (a edgeHasManyAttrs) WithContext(ctx context.Context) *edgeHasManyAttrs {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a edgeHasManyAttrs) Session(session *gorm.Session) *edgeHasManyAttrs {
	a.db = a.db.Session(session)
	return &a
}

func (a edgeHasManyAttrs) Model(m *model.Edge) *edgeHasManyAttrsTx {
	return &edgeHasManyAttrsTx{a.db.Model(m).Association(a.Name())}
}

type edgeHasManyAttrsTx struct{ tx *gorm.Association }

func (a edgeHasManyAttrsTx) Find() (result []*model.EdgeAttr, err error) {
	return result, a.tx.Find(&result)
}

func (a edgeHasManyAttrsTx) Append(values ...*model.EdgeAttr) (err error) {
...
func (a edgeHasManyAttrsTx) Replace(values ...*model.EdgeAttr) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a edgeHasManyAttrsTx) Delete(values ...*model.EdgeAttr) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a edgeHasManyAttrsTx) Clear() error {
	return a.tx.Clear()
}

func (a edgeHasManyAttrsTx) Count() int64 {
	return a.tx.Count()
}
